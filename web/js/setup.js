...
async function start(control, player) {
	...
	for (i in sequence) {
		if (socket.readyState !== 1 || interrupted)
...
function updateProgress(status, pr, max, pause) {
	if (status > 0) {
		...
		switch (status) {
			case 1: $progress.classList.remove('complete', 'stopped'); break
			case 2: $progress.classList.add('complete'); break
			case 3: $progress.classList.add('stopped'); break
		}
		if (status >= 2) {
