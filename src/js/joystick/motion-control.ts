...
export default function loadDriveMode() {
	const drive = Controller.elements.buttons.find(
	...
	function toggleDriveMode(e) {
		...
		window.ondevicemotion = (e) => {
			...
			const inverted =
				e.accelerationIncludingGravity[land ? 'x' : 'y'] >= 0 ? 1 : -1
			driveY = parseFloat(
				e.accelerationIncludingGravity[land ? 'y' : 'x'].toFixed(1)
			)
			...
			if (!options.vgamepad) {
				...
				const unpressKeys = lastDirections.filter(
