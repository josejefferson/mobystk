...
def installVgamepad():
	...
	try:
		...
		if not spec: raise Exception('O módulo vgamepad não está instalado')
		arch = getArch()
		vigemSetupPath = os.path.normpath(os.path.join(spec.origin, f'../win/vigem/install/{arch}/ViGEmBusSetup_{arch}.msi'))
		exitCode = subprocess.call(['msiexec', '/i', vigemSetupPath])
		if exitCode == 0: reloadScript('O controle virtual foi instalado!')
		else: raise Exception(f'Comando retornou o código de erro {exitCode}')
	except:
